...
curl -XPOST -d "iglu_server_super_uuid=$iglu_server_super_uid" localhost:10000/addigluserversuperuuid

#add domain name to Caddyfile
curl -XPOST -d "tls_status=$tls_status&domain_name=$domain_name" localhost:10000/adddomainname

#add username and password to Caddyfile for basic auth
curl -XPOST -d "new_username=$username&new_password=$password" localhost:10000/changeusernameandpassword
